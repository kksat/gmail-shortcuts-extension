...
**Auto-activating:** You do not need to configure any trigger keys. The extension automatically detects when Gmail's Snooze menu appears (whether opened via Gmail's native shortcut like `b`, clicking the Snooze button, or from the right-click menu) and enables instant single-key actions.
...
Once the Snooze menu is visible on screen:
